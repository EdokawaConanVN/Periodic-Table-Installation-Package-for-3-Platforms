...
Write-Host "Installing Bảng Tuần Hoàn Nguyên Tố Hóa Học v1.8.4..."
$currentDir = Get-Location
$msixPath = ($currentDir.Path + "\" + "Bảng Tuần Hoàn Nguyên Tố Hóa Học v1.8.4.sideload.msix")
...
if ($installProc.ExitCode -eq 0) {
	$app = Get-StartApps "Bảng Tuần Hoàn Nguyên Tố Hóa Học v1.8.4"

	# If it's an array, then we found multiple matching apps. Grab the last one.
	if ($app -is [array]) {
		Write-Host "Warning: found multiple apps installed named Bảng Tuần Hoàn Nguyên Tố Hóa Học v1.8.4. Launching best guess. If the wrong app launches, find the right one in your start menu."
		$app = $app[-1];
	}

	if ($app) {
		Write-Host "Launching Bảng Tuần Hoàn Nguyên Tố Hóa Học v1.8.4..."
